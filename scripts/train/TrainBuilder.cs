...
public partial class TrainBuilder : Node3D
{
    ...
    public float CabooseZ    { get; private set; }
    ...
    // ClampSetup is declared as a top-level enum (see bottom of this file) so ClampNode can also reference it.
    ...
    private void AttachContainers(Carriage carriage, int slotsPerSide, RandomNumberGenerator rng)
    {
    ...
    private void AttachClampsForSetup(ContainerNode container, bool isRightSide, ClampSetup setup, RandomNumberGenerator rng)
    {
        ...
        foreach (var pos in positions)
        {
            ...
            clampInstance.Configure(setup, ComputeSurfaceNormal(pos));
    ...
    /// <summary>
    /// Determines which face of the container a clamp sits on, based on its local position.
    /// The dominant axis (largest absolute component) is taken as the outward normal.
    /// </summary>
    private static Vector3 ComputeSurfaceNormal(Vector3 localPos)
    {
        float ax = Mathf.Abs(localPos.X);
        float ay = Mathf.Abs(localPos.Y);
        if (ax >= ay)
            return new Vector3(Mathf.Sign(localPos.X), 0f, 0f);
        return new Vector3(0f, Mathf.Sign(localPos.Y), 0f);
    }
    ...
    private static Node3D CreateBoxCar(string name, Vector3 size, Color color, string? glbPath = null)
    {
        ...
        if (loadedMesh != null)
        {
            ...
            meshSlot.MaterialOverride = new StandardMaterial3D { AlbedoColor = color };
        }
        else
        {
...
/// <summary>
/// The four clamp arrangements used on containers.
/// Declared at top level so both TrainBuilder and ClampNode can reference it.
/// </summary>
public enum ClampSetup { Single, Double, Triple, Four }
